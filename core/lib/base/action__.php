...
class ActionUtil {
	...
	public static function numOfShellArgs() {
		return array (
			...
			'Access'=>	array(
				/*
				 * http://localhost:8080/vfs/?xxx=access|1-2-3。
				 * 这个方法的访问权限比较特殊。
				 * 因为不输入指令的时候，只要有后续参数，那么，就会执行此方法。
				 * 也就是说，该方法的指令“access”是URL的默认指令。
				 * 缺省的URL可以写成：http://localhost:8080/vfs/?xxx=1-2-3
				 */
				'access'	=>	1,
				'accessDirToContentHTML'=>	1,
				'updatePathNavyInAdminViewFormIdArray'	=>	1,
				'orientateDirFromIdAndGenerateListHTML'	=>	1,
			),
			'Admin'	=>	array(
				'admin'			=>	0,
				'adminLogin'	=>	0,
	...
	*/
	...
	/**
	 * 自动检测开发者在  指定的  目录自行添加的库文件或目录，并包含其内任何层次目录的[*.php]。
