...
if ( UrlUtil::URIInterceptor ( $_SERVER['REQUEST_URI'] ) ){
	...
	require_once APPROOT.PAGE_404;
	die;
}
/*
 * 分析URL参数
 */
$urlInfo = UrlUtil::analyseUrlParam($_REQUEST);
$_SESSION['urlInfo'] = $urlInfo;	//将分析结果加入会话
// var_dump($urlInfo);
/*
...
// 	var_dump($urlInfo);
	...
	* 现取URL参数状态不为2的情况(3和5状态)所达到的分支，作为普通模板输出！
	* 剩下的URL参数状态2：将在【else if(2 == $urlInfo['status'])】分支用作xml的id来读取，最终来访问虚拟文件系统。
	...
	* 接下来的就是页面[模板]输出的代码：
	*/
	if(2 != $urlInfo['status'] && null != $urlInfo['params'] && 0 != count($urlInfo['params']) && null != $urlInfo['params'][0]){
	...
	}else if(2 == $urlInfo['status']){
		//这里通过取到的id值访问虚拟文件系统
		ActionUtil::action('Access')->analyseIdArrayToDirOrFile($urlInfo['ids']);
// 		AccessAction::analyseIdArrayToDirOrFile($urlInfo['ids']);
		exit;
	}
}else{
	exit;
}
// echo "请求的页面不存在，这里可以设计404页面";

require APPROOT.PAGE_404;
exit;
